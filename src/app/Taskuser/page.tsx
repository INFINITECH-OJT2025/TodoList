...
const ActivityPage = () => {
  ...
  const fetchActivities = async () => {
    try {
      ...
      const response = await axios.get(`https://infinitech-api5.site/activities/${authToken}`);
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get(`https://infinitech-api5.site/users`); // Adjust the endpoint as necessary
  ...
  useEffect(() => {
    ...
    const userIdSetter = async () => {
      const authToken = sessionStorage.getItem("authToken");
      const userResponse = await axios.get(`https://infinitech-api5.site/user/${authToken}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(`https://infinitech-api5.site/activities/${editId}`, newFormData);
        toast.success("Activity updated successfully!"); // Use toast for success message
      } else {
        await axios.post(`https://infinitech-api5.site/activities`, newFormData);
  ...
  const handleDeleteConfirm = async () => {
    if (selectedId !== null) {
      try {
        await axios.delete(`https://infinitech-api5.site/activities/${selectedId}`);
  ...
  const handleMarkAsDone = async (id: number) => {
    try {
      await axios.put(`https://infinitech-api5.site/activities/${id}/done`);
  ...
  const handleArchiveConfirm = async () => {
    if (selectedId !== null) {
      try {
        await axios.put(`https://infinitech-api5.site/activities/${selectedId}/archive`);
  ...
  const handleRestore = async (id: number) => {
    try {
      await axios.put(`https://infinitech-api5.site/activities/${id}/restore`);
  ...
  const handleOverdue = async (id: number) => {
    try {
      ...
      const response = await axios.put(`https://infinitech-api5.site/activities/${id}/overdue`);
